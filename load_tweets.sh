...
for file in $files; do
		python3 -u load_tweets.py --db postgresql://postgres:pass@localhost:1833 --inputs $file
	# call the load_tweets.py file to load data into pg_normalized
done

echo 'load denormalized'
for file in $files; do
	unzip -p $file | sed 's/\\u0000//g' | psql postgresql://postgres:pass@localhost:1832 -c "COPY tweets_jsonb (data)  FROM STDIN csv quote e'\x01' delimiter e'\x02';"
done
